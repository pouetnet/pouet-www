...
class PouetBoxAdminEditProd extends PouetBoxSubmitProd
{
    ...
    public function Commit($data)
    {
        ...
        $groups = array();
        if ($data["group1"]) {
            $groups[] = (int)$data["group1"];
        }
        if ($data["group2"]) {
            $groups[] = (int)$data["group2"];
        }
        if ($data["group3"]) {
            $groups[] = (int)$data["group3"];
        }
        $groups = array_unique($groups);
        if (count($groups)) {
            $a["group1"] = array_shift($groups);
        } else {
            $a["group1"] = null;
        }
        if (count($groups)) {
            $a["group2"] = array_shift($groups);
        } else {
            $a["group2"] = null;
        }
        if (count($groups)) {
            $a["group3"] = array_shift($groups);
        } else {
            $a["group3"] = null;
        }
...
document.observe("dom:loaded",function(){
  $("pouetbox_proddelete").up("form").observe("submit",function(e){
    ...
    if (!confirm("ARE YOU REALLY SURE YOU WANT TO DELETE " + <?=json_encode($this->prod->name, JSON_HEX_TAG)?> + "?!"))
      e.stop();
  });
});
</script>
...
class PouetBoxAdminEditProdAwards extends PouetBoxEditConnectionsBase
{
    ...
    public function RenderEditRow($row = null)
    {
    ...
    public function RenderNormalRow($v)
    {
        global $AWARDS_CATEGORIES;
