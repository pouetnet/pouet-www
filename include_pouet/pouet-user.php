...
class PouetUser extends BM_Class
{
    ...
    {
        return array("id","nickname","level","permissionSubmitItems","permissionPostBBS","permissionOpenBBS","permissionPostOneliner","avatar","glops","registerDate","lastLogin","thumbups","thumbdowns");
    }
    public static function getExtendedFields()
    {
        return array("udlogin","sceneIDLastRefresh","sceneIDData","ojuice","slengpung","csdb","zxdemo","demozoo","lastip","lasthost");
    }

    public function PrintAvatar()
    {
    ...
    public function PrintLinkedAvatar()
    {
        return sprintf("<a href='user.php?who=%d' class='usera' title=\"%s\">%s</a>", $this->id, _html($this->nickname), $this->PrintAvatar());
    }
    public function PrintLinkedName()
    {
        $classes = array("user");
        if ($this->lastLogin && (time() - strtotime($this->lastLogin)) < 5 * 60) {
            $classes[] = "online";
        }
        if ($this->IsBanned()) {
            $classes[] = "banned";
        ...
        if ($thumbCount > 10000) {
            $classes[] = "badge-platinum";
        } elseif ($thumbCount >  5000) {
            $classes[] = "badge-gold";
        } elseif ($thumbCount >  1000) {
            $classes[] = "badge-ruby";
        } elseif ($thumbCount >   500) {
            $classes[] = "badge-silver";
        } elseif ($thumbCount >   100) {
            $classes[] = "badge-bronze";
        }
    ...
    public function CalculateGlops()
    {
        $glops = 0;
    ...
    public function GetSceneIDData($cached = true)
    {
    ...
    public function CanPostInOneliner()
    {
        return $this->permissionPostOneliner != 0;
    }
    public function CanOpenNewBBSTopic()
    {
        return $this->permissionOpenBBS != 0;
    }
    public function CanPostInBBS()
    {
        return $this->permissionPostBBS != 0;
    }
    public function CanPostInProdComments()
    {
        return true;
    }
    public function CanSubmitItems()
    {
