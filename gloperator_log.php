...
class PouetBoxGloperatorLog extends PouetBox
{
    ...
    public function RenderBody()
    {
        ...
        echo "<div class='foot'>";
        switch ($_GET["what"]) {
            case "prod": printf("<a href='prod.php?which=%d'>back to the prod</a>", $_GET["which"]);
                break;
            case "group": printf("<a href='groups.php?which=%d'>back to the group</a>", $_GET["which"]);
                break;
            case "topic": printf("<a href='topic.php?which=%d'>back to the topic</a>", $_GET["which"]);
                break;
            case "party": printf("<a href='party.php?which=%d'>back to the party</a>", $_GET["which"]);
                break;
        }
        echo "</div>\n";
    }
