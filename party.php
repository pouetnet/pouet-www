...
class PouetBoxPartyHeader extends PouetBox
{
    ...
    public $year;
    public $years;
    public function __construct($partyView)
    {
        parent::__construct();
        ...
        foreach ($this->prods as $p) {
            if ($p->party_compo != $lastCompo && !$headerDone) {
                ...
                foreach ($headers as $key => $text) {
                    ...
                    if ($key == "compo" && $this->sortByCompo && $p->party_compo && $COMPOTYPES[$p->party_compo]) {
                        $out = sprintf("<th id='%s'>%s</th>", hashify($COMPOTYPES[$p->party_compo]), $COMPOTYPES[$p->party_compo]);
                    }
                    echo $out;
                }
                echo "</tr>\n";
