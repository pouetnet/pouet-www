...
class BM_Node
{
    ...
    public $prefix;
    public $fieldFunc;
    public function __construct(&$query, $a = array())
    {
        global $BM_ORM_CLASSES;
    ...
    public function &attach(&$query, $local, $relations, $additionalCondition = null)
    {
        global $BM_ORM_CLASSES;
        ...
        if (!$relationTableAlias) {
            $relationTableAlias = $relationTableCanonical;
        }
    ...
    public function getTableInstanceName()
    {
        return $this->prefix ? $this->prefix : $this->table;
    }
    public function addExtendedFields(&$query)
    {
        $class = $this->class;
...
class BM_Query extends SQLSelect
{
    ...
    public function perform() // "'perform'?! in germany, queries get EXECUTED!" /dfox/
    {
        $objects = array();
        $sql = $this->GetQuery();
    ...
    public function performWithCalcRows(&$count)
    {
        $objects = array();
        $sql = $this->GetQuery();
...
class BM_Class
{
    ...
    {
        trigger_error("GetTable not overridden in ".get_class(), E_USER_ERROR);
    }
    ...
    {
        return array_merge(static::getFields(), array());
    }
