...
class PouetBoxUserlist extends PouetBox
{
    ...
    public function LoadFromDB()
    {
        ...
        $dir = !@$_GET["reverse"];
        switch (@$_GET["order"]) {
            case "nickname": $s->AddOrder("users.nickname ".($dir ? "ASC" : "DESC"));
                break;
            case "age": $s->AddOrder("users.registerDate ".($dir ? "ASC" : "DESC"));
                break;
            case "level": $s->AddOrder("users.level ".($dir ? "ASC" : "DESC"));
                break;
            case "glops":
            default: $s->AddOrder("users.glops ".($dir ? "DESC" : "ASC"));
                break;
        }
    ...
    public function Render()
    {
        ...
        foreach ($headers as $key => $text) {
            $out = sprintf(
                "<th><a href='%s' class='%s%s' id='%s'>%s</a></th>\n",
