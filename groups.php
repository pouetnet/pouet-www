...
class PouetBoxGroupMain extends PouetBox
{
    ...
    public function LoadFromDB()
    {
        ...
        $r = !!@$_GET["reverse"];
        switch (@$_GET["order"]) {
            case "type": $s->AddOrder("prods.type ".($r ? "DESC" : "ASC"));
                break;
            case "platform": { $s->AddJoin("left", "prods_platforms as pp", "pp.prod=prods.id");
                $s->AddOrder("pp.platform ".($r ? "DESC" : "ASC"));
                break; }
            case "party": $s->AddOrder("prods_party.name ".($r ? "DESC" : "ASC"));
                $s->AddOrder("prods.party_year ".($r ? "DESC" : "ASC"));
                $s->AddOrder("prods.party_place ".($r ? "DESC" : "ASC"));
                break;
            case "release": $s->AddOrder("prods.releaseDate ".($r ? "ASC" : "DESC"));
                break;
            case "thumbup": $s->AddOrder("prods.voteup ".($r ? "ASC" : "DESC"));
                break;
            case "thumbpig": $s->AddOrder("prods.votepig ".($r ? "ASC" : "DESC"));
                break;
            case "thumbdown": $s->AddOrder("prods.votedown ".($r ? "ASC" : "DESC"));
                break;
            case "avg": $s->AddOrder("prods.voteavg ".($r ? "ASC" : "DESC"));
                break;
            case "views": $s->AddOrder("prods.views ".($r ? "ASC" : "DESC"));
                break;
            case "latestcomment": $s->AddOrder("lastcomment ".($r ? "ASC" : "DESC"));
                break;
            default: $s->AddOrder("prods.name ".($r ? "DESC" : "ASC"));
                break;
        }
    ...
    public function Render()
    {
        ...
        if ($this->group->csdb) {
            echo sprintf(" [<a href='https://csdb.dk/group/?id=%d'>csdb</a>]", $this->group->csdb);
        }
